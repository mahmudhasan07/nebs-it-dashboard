...
import { useGetDashboardInfoQuery } from "@/Redux/Api/userApi";
// import { useGetAllEventsQuery } from "@/Redux/Api/eventApi";

export default function DashboardOverview() {
  const [selectedValue, setSelectedValue] = useState<string>('this-month');
  const {totalBooking, todayTotalBooking} = useGetDashboardInfoQuery("", {
    selectFromResult: ({ data }) => ({
      totalBooking: data?.data?.totalBooking,
      todayTotalBooking: data?.data?.todayTotalBooking
    })
  })
      ...
      <div className="max-w-[1400px] mx-auto">
        <div className="grid lg:grid-cols-4 grid-cols-2 gap-6 justify-between">
          ...
          <div className="w-full bg-white rounded-xl shadow-md border">
            <div className="relative p-6  ">
              <div className="space-y-4 font-poppins">
                <h3 className="text-xl font-medium text-gray-900">Total Booking</h3>
                <div className="flex items-baseline gap-2">
                  <span className="text-[32px] font-bold tracking-tight"> <CountUp end={totalBooking} /></span>
          ...
          <div className="w-full bg-white rounded-xl shadow-md border">
            <div className="relative p-6 ">
              <div className="space-y-4 font-poppins">
                <h3 className="text-xl font-medium text-gray-900">Toady New Booking</h3>
                <div className="flex items-baseline gap-2">
                  <span className="text-[32px] font-bold tracking-tight"><CountUp end={todayTotalBooking} duration={3}></CountUp></span>
